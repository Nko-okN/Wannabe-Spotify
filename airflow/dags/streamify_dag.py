...
BIGQUERY_DATASET = os.environ.get('BIGQUERY_DATASET', 'wannabe-spotify_stg')
...
with DAG(
    dag_id = f'wannabe-spotify_dag',
    default_args = default_args,
    description = f'Hourly data pipeline to generate dims and facts for wannabe-spotify',
    schedule_interval="5 * * * *", #At the 5th minute of every hour
    start_date=datetime(2022,3,26,18),
    catchup=False,
    max_active_runs=1,
    user_defined_macros=MACRO_VARS,
    tags=['wannabe-spotify']
) as dag:
    ...
    execute_dbt_task = BashOperator(
        task_id = 'dbt_wannabe-spotify_run',
